...
public class BondRepository {
    ...
    public List<Bond> findTopByAnnualYieldAndMaturityRangeWithSearch(int limit, int minWeeksToMaturity, int maxWeeksToMaturity, boolean useOfferYield, String searchText) {
        // Подготавливаем условие поиска
        String searchCondition = "";
        if (searchText != null && !searchText.trim().isEmpty()) {
            searchCondition = " AND (UPPER(ticker) LIKE UPPER(?) OR UPPER(short_name) LIKE UPPER(?)) ";
        }
        ...
        if (!useOfferYield) {
            ...
            if (searchText != null && !searchText.trim().isEmpty()) {
                ...
                return jdbcTemplate.query(sql, bondRowMapper, searchPattern, searchPattern, limit);
            } else {
                return jdbcTemplate.query(sql, bondRowMapper, limit);
            }
        }
